...
std::string Markdown::findCoverImage(const std::function<bool()>& shouldAbort) const {
...
bool Markdown::generateCoverBmp(bool use1BitDithering, const std::function<bool()>& shouldAbort) const {
  ...
  const bool success = CoverHelpers::convertImageToBmp(coverImagePath, coverPath, "MD ", use1BitDithering, shouldAbort);
  ...
  if (cancelled) {
    ...
    FsFile marker;
    if (SdMan.openFileForWrite("MD ", failedMarkerPath, marker)) marker.close();
  } else {
    SdMan.remove(failedMarkerPath.c_str());
  }
  return valid;
