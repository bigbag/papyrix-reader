...
Viewport dimensions are computed dynamically from `GfxRenderer::getScreenWidth()` / `getScreenHeight()`, which read the actual panel size from `EInkDisplay`. The same code serves both panels.
