...
int main() {
  ...
  // Test malformed and truncated sequences
  {
    constexpr uint32_t replacement = UTF8_REPLACEMENT_CODEPOINT;

    const unsigned char truncated2[] = {0xC3, 0x00};
    const unsigned char* ptr = truncated2;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr), "utf8NextCodepoint: truncated 2-byte returns replacement");
    runner.expectEq(size_t{1}, static_cast<size_t>(ptr - truncated2),
                    "utf8NextCodepoint: truncated 2-byte advances one");

    const unsigned char truncated3[] = {0xE4, 0xB8, 0x00};
    ptr = truncated3;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr), "utf8NextCodepoint: truncated 3-byte returns replacement");
    runner.expectTrue(ptr > truncated3, "utf8NextCodepoint: truncated 3-byte advances");

    const unsigned char truncated4[] = {0xF0, 0x9F, 0x00};
    ptr = truncated4;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr), "utf8NextCodepoint: truncated 4-byte returns replacement");

    const unsigned char overlong[] = {0xC0, 0x80, 0x00};
    ptr = overlong;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr), "utf8NextCodepoint: rejects overlong encoding");

    const unsigned char surrogate[] = {0xED, 0xA0, 0x80, 0x00};
    ptr = surrogate;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr), "utf8NextCodepoint: rejects surrogate");

    const unsigned char tooLarge[] = {0xF4, 0x90, 0x80, 0x80, 0x00};
    ptr = tooLarge;
    runner.expectEq(replacement, utf8NextCodepoint(&ptr),
                    "utf8NextCodepoint: rejects codepoint above Unicode range");
  }
