...
def locale_data(path: Path) -> tuple[set[str], int, dict[str, str]]:
    keys = set()
    values = {}
    used = 0
...
def main() -> int:
    ...
    for locale in sorted(LOCALE_DIR.glob("*.txt")):
        actual, used, values = locale_data(locale)
        missing = sorted(expected - actual)
        unexpected = sorted(actual - expected)
        capacity_exceeded = used > BUFFER_SIZE
        orientation_ambiguous = (
            "LANDSCAPE_CW" in values
            and "LANDSCAPE_CCW" in values
            and values["LANDSCAPE_CW"] == values["LANDSCAPE_CCW"]
        )
        if missing or unexpected or capacity_exceeded or orientation_ambiguous:
            failed = True
            ...
            if orientation_ambiguous:
                print("  LANDSCAPE_CW and LANDSCAPE_CCW must have distinct labels")
        else:
